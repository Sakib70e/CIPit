...
export const InventoryScreen = () => {
  ...
  const handleAdjustStock = async () => {
    const num = parseInt(adjustModal.manualAmount);
    if (isNaN(num) || num <= 0) {
    ...
    try {
      ...
      setAdjustModal({ ...adjustModal, visible: false, manualAmount: '' });
      fetchInventory();
    } catch (err) {
      Alert.alert('Protocol Error', err.response?.data?.message || 'Stock adjustment rejected.');
    }
  };
  ...
  const handleAddItem = async () => {
    ...
    try {
      ...
      setNewItem({ itemName: '', size: '', price: '', totalStock: '', imageUri: null });
      fetchInventory();
    } catch (err) {
  ...
  const renderItem = ({ item }) => {
    const isLowStock = item.availableStock < 20;
    return (
        ...
        <View style={styles.cardBody}>
           ...
           <View style={styles.stockPanel}>
              ...
              <View style={styles.stockCol}>
                 ...
                 <Text style={styles.stockVal}>{item.reservedStock}</Text>
              </View>
              <View style={styles.stockCol}>
           ...
           <View style={styles.cardActions}>
              <TouchableOpacity style={styles.adjustBtn} onPress={() => setAdjustModal({ visible: true, item, manualAmount: '', mode: 'ADD' })}>
      ...
      <Modal visible={adjustModal.visible} transparent={true} animationType="fade">
        <View style={styles.modalOverlay}>
          <View style={styles.modalContent}>
            ...
            <Text style={styles.currentSummary}>Current Base: {adjustModal.item?.totalStock} Units</Text>
            
            <View style={styles.manualInputGroup}>
               <Text style={styles.label}>ENTER UNIT QUANTITY</Text>
               <TextInput 
            ...
            <View style={styles.modeControlRow}>
               ...
               >
                  <Ionicons name="add-circle" size={24} color={adjustModal.mode === 'ADD' ? COLORS.white : COLORS.success} />
                  <Text style={[styles.modeBtnText, adjustModal.mode === 'ADD' && styles.modeBtnTextActive]}>INCOMING</Text>
               </TouchableOpacity>
               ...
               >
                  <Ionicons name="remove-circle" size={24} color={adjustModal.mode === 'SUB' ? COLORS.white : COLORS.danger} />
                  <Text style={[styles.modeBtnText, adjustModal.mode === 'SUB' && styles.modeBtnTextActive]}>OUTGOING</Text>
               </TouchableOpacity>
            </View>
            ...
            <View style={styles.modalActions}>
               ...
               <TouchableOpacity style={styles.modalCommit} onPress={handleAdjustStock}>
                  <Text style={styles.commitText}>COMMIT CHANGES</Text>
               </TouchableOpacity>
            </View>
          </View>
        </View>
...
const styles = StyleSheet.create({
  ...
  itemRef: { fontSize: 14, fontWeight: '700', color: COLORS.textSecondary, textAlign: 'center', marginBottom: 4 },
  currentSummary: { fontSize: 13, fontWeight: '800', color: COLORS.primary, textAlign: 'center', marginBottom: 25 },
  
  manualInputGroup: { marginBottom: 15 },
  manualInput: { backgroundColor: COLORS.light, borderRadius: 16, height: 60, textAlign: 'center', fontSize: 28, fontWeight: '900', color: COLORS.dark },
  
  modeControlRow: { flexDirection: 'row', justifyContent: 'space-between', marginBottom: 20 },
  modeBtn: { flex: 1, flexDirection: 'row', alignItems: 'center', justifyContent: 'center', backgroundColor: COLORS.light, paddingVertical: 12, borderRadius: 14, marginHorizontal: 5 },
